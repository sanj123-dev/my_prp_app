...
export default function GoalPlannerScreen() {
  ...
  const hydrate = (payload: GoalPlannerV2Progress, resetChat = false) => {
    ...
    const assistantBase = `${payload.assistant_message ?? ''}`.trim();
    const promptText = `${payload.next_prompt?.prompt ?? ''}`.trim();
    let assistantText = assistantBase;
    if (!assistantText && promptText) assistantText = promptText;
    if (assistantText && promptText) {
      const normalizedBase = assistantText.toLowerCase();
      const normalizedPrompt = promptText.toLowerCase();
      if (!normalizedBase.includes(normalizedPrompt)) {
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.subtitle}>Dynamic planner based on your real finances</Text>
        </View>
        <TouchableOpacity style={styles.iconBtn} onPress={() => void bootstrap(true)}>
